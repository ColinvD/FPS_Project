...
public class InsertShot : MonoBehaviour {
    ...
    {
        WWW request = new WWW("http://22545.hosts.ma-cloud.nl/bewijzenmap/p2.1/gpr/dataBaseScripts/insertLogShots.php?" +
            "obj" + obj_name.ToString() +
    ...
    public IEnumerator DoPHP(string obj_name, Vector3 beg, Vector3 end, Vector3 rot, int playerID)
    {
        WWW request = new WWW("http://22545.hosts.ma-cloud.nl/bewijzenmap/p2.1/gpr/dataBaseScripts/insertLogShots.php?" +
            "obj" + obj_name.ToString() +
            "&posxbeg=" + beg.x.ToString() +
            ...
            "&player_id=" + playerID.ToString());
        yield return request;
        Debug.Log("shot inserted" + request.text);
